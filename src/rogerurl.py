...
def run_script():
    ...
    for new_point in url_test:
        print(f"current point{new_point}")
        clf.fit(points)
        ...
        if prediction == category_labels[int(new_point[-1])]:
            correct_predictions += 1
        print(f"Current accuracy progress: {correct_predictions}/{len(url_test)}")
